...
Http2Adapter.prototype._resolveSessionFor = function(origin, isStreaming) {
  ...
  if (!this._sessionMap[sessionKey]) {
    ...
    this._sessionMap[sessionKey] = _http.default
      .connect(origin)
      .once('error', cleanup)
      .once('goaway', cleanup) // Destroys http2 session after specified time of inactivity
...
Http2Adapter.prototype.execute = function(options) {
  ...
  return new Promise(function(resolvePromise, rejectPromise) {
    ...
    var onAbort = function() {
      isCanceled = true
      cleanup()
      request.close(_http.default.constants.NGHTTP2_CANCEL)
      rejectOrOnError(new _errors.AbortError())
    }
    ...
    var onTimeout = function() {
      ...
      cleanup()
      request.close(_http.default.constants.NGHTTP2_CANCEL)
      rejectOrOnError(new _errors.TimeoutError())
    }

    var onResponse = function(responseHeaders) {
      var status = responseHeaders[_http.default.constants.HTTP2_HEADER_STATUS]
    ...
    try {
      var pathname =
        ...
        (0, _util.querystringify)(options.query, '?')
      var requestHeaders = Object.assign({}, options.headers, {
        [_http.default.constants.HTTP2_HEADER_PATH]: pathname,
        [_http.default.constants.HTTP2_HEADER_METHOD]: options.method,
      })
      ...
      var request = session
        ...
        .setEncoding('utf8')
        .on('error', onError)
        .on('response', onResponse) // Set up timeout only if no signal provided.
