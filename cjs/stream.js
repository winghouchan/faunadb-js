...
var _Get = _interopRequireDefault(require('./query/Get'))
...
function StreamClient(client, expression, options, onEvent) {
  ...
  this._query = (0, _common.wrap)(expression)
...
StreamClient.prototype.snapshot = function() {
  ...
  self._client
    .query((0, _Get.default)(self._query))
    .then(function(doc) {
      self._onEvent({
        type: 'snapshot',
        event: doc,
...
StreamClient.prototype.subscribe = function() {
  ...
  function onResponse(response) {
    ...
    try {
      parsed = (0, _json.parseJSON)(response.body)
    } catch (_) {
  ...
  function onData(data) {
    var result = (0, _json.parseJSONStreaming)(buffer + data)
    ...
    result.values.forEach(function(event) {
      ...
      if (event.event === 'error') {
        onError(new _errors.StreamErrorEvent(event))
      } else {
        self._onEvent(event)
      }
    })
